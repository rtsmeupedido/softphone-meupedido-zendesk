...
import { MuiIcon as Icon, Input } from "rtk-ux";
import {
  Container,
  ItemDetails,
  ItemName,
    ...
    async function fetchHistoric() {
      ...
      try {
        const response = await api.post(
          "/api/_pbx_cdr/filter?$type=query",
          data
        );
    ...
    <Container>
      <StickyHeader>
        ...
        {search && (
          <SearchResults>
            Resultados para{" "}
            <span className="text-bold text-slate-600">{search}</span>
          </SearchResults>
        )}
      </StickyHeader>
                  ...
                  <div className="flex items-center justify-center">
                    {item.disposition === "ANSWERED" ? (
                      <Icon
                        ...
                        icon={["mui", "phone_callback"]}
                      />
                    ) : item.disposition === "NO ANSWER" ? (
                      <Icon
                        className="text-red-500"
                        icon={["mui", "phone_enabled"]}
                      />
                    ) : (
                      <Icon
                        className="text-yellow-500"
                        icon={["mui", "phone_disabled"]}
                      />
                    )}
                  </div>
                  <div className="ml-3 flex justify-between w-full">
                    <div className="text-center items-center flex text-sm">
                      {item.disposition === "ANSWERED" ? item.src : item.dst}
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <div className="text-xs text-zinc-500 font-light">
                        {formatDate(
                          ...
                          item.calldate < new Date().setHours(0, 0, 0, 0)
                            ? "HH:mm"
                            : "dd/MM/yyyy hh:mm"
                        )}
                      </div>
