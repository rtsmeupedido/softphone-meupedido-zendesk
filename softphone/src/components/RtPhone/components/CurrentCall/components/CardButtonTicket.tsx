...
const CardButtonTicket = ({ disabled, ticketId, id }: Props) => {
  ...
  const user_phone =
    currentCall[0]?._remote_identity?._uri?._user || "Usuário Desconhecido";
  const display_name = currentCall[0]?._remote_identity?._display_name;
  const date_and_time = currentCall[0]?._start_time
    ? new Date(currentCall[0]._start_time)
    : null;
  const call_id =
    currentCall[0]?._request?.ua?._registrator?._call_id || "ID Desconhecido";
  ...
  async function handleAdcTicket(id: number) {
    const formattedDate =
      date_and_time && isValid(date_and_time)
        ? format(date_and_time, "dd/MM/yyyy HH:mm")
        : "Data inválida";
