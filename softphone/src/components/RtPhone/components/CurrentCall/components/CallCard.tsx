...
const CallCard = ({
  ...
  const isMuted = useAppSelector((state) => {
    const call = state.softphone.data.find(
      (call) => call.id === currentCall.id
    );
        ...
        <>
          <Col span={24}>
            <div className="flex flex-col items-center gap-1">
              {paused ? (
                <div className="text-yellow-600">Ligação pausada</div>
              ) : (
              ...
              <Tag color={paused ? "gold" : "blue"}>
                {currentCall?.start_time ? (
                  <StopWatch
                    isPaused={paused}
                    datetime={currentCall?.start_time}
                  />
                ) : (
                  "00:00"
                )}
            ...
            {tenantSettingsOrganizationsId?.enable_mute_button && (
              <Col>
                <Tooltip
                  title={
                    isTooltipVisible
                      ? "Ativo somente após 10 segundos"
                      : `${isMuted ? "Ativar" : "Desativar"} microfone`
                  }
                >
                  <Button
                    block
                    type={currentCall?.muted ? "dashed" : "default"}
                    onClick={() => toggleMute(currentCall)}
                    disabled={isButtonDisabled}
                    className={
                      isButtonDisabled ? "cursor-not-allowed" : "pointer"
                    }
                  >
                    <Icon
                      icon={[
                        "fas",
                        currentCall?.muted ? "microphone-slash" : "microphone",
                      ]}
                    />
                  </Button>
                </Tooltip>
            ...
            {tenantSettingsOrganizationsId?.enable_waitcall_button && (
              <Col>
                <Tooltip title={`${isHolded ? "Retomar" : "Pausar"} chamada`}>
                  <Button
                    block
                    type={isHolded ? "dashed" : "default"}
                    onClick={() => toggleBreak(currentCall)}
                  >
                    <Icon icon={["fas", isHolded ? "play" : "pause"]} />
                  </Button>
                </Tooltip>
              </Col>
            )}
          ...
          <Col span={24}>
            <Button
              block
              danger
              onClick={() => onChangToSwitchOff(currentCall)}
            >
              Desligar
            </Button>
          </Col>
            ...
            <div className="flex flex-col gap-4 p-4 bg-white shadow-md rounded-md w-full">
              ...
              <div className="flex justify-between">
                <Button
                  ...
                  type="primary"
                  className="flex-1 text-sm rounded-lg mr-2"
                  onClick={() =>
                    handleTransferConsult(inputValue, currentCall.id)
                  }
                >
                  Direta
                </Button>
                <Button
                  ...
                  onClick={() => handleAssistedTransfer(inputValue)}
                >
                  Com consulta
                </Button>
              </div>
