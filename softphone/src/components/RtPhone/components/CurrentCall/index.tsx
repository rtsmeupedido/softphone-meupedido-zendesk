...
export default function CurrentCall({ currentCalls, isPaused, isMuted, startCall, tenantSettingsOrganizationsId }: any) {
                ...
                .map((currentCall: any, index: any) => {
                    return (
                        <div className="" key={currentCall?.id || index}>
                            <Row key={currentCall?.id || index} gutter={[8, 8]} justify={"center"} className="relative -mx-3">
                                {index > 0 && <Divider className="my-1" />}
                                <CallCard currentCall={currentCall} key={currentCall.id} isPaused={isPaused} isMuted={isMuted} startCall={startCall} index={index} tenantSettingsOrganizationsId={tenantSettingsOrganizationsId} />
                                ...
                                {showNewUser ? (
                                    <>
                                        ...
                                        <Col span={24}>
                                            <Input value={currentCall?.remote_identity.uri.user} disabled placeholder="Telefone" />
                                        </Col>
                                        <Col span={12}>
                                        ...
                                        <Col span={12}>
                                            <Button block type="primary" onClick={() => onAddContact(currentCall)}>
                                                Adicionar contato
                                            </Button>
                                        </Col>
                                    </>
                                            ...
                                            {selectedUserId ? (
                                                <div className="user-detail">
                                                    {ticket?.users?.length &&
                                                        ...
                                                        ticket.users.find((user) => user.id === selectedUserId) &&
                                                        ticket.users.filter((user) => user.id === selectedUserId).map((user) => <CartListTicket key={user.id} handleBackClick={handleBackClick} userId={user.id} />)}
                                                </div>
                                            ) : (
                                                <>
                                                    {ticket?.users?.length && ticket?.users?.length > 0 && ticket?.users?.length > 0 ? (
                                                        <>
                                                            <Col span={24} className="mt-3">
                                                                <div className="mb-2">Foram encontrados estes contatos:</div>
                                                        ...
                                                        <Col span={24} className="text-center text-gray-500 py-8 text-xs">
                                                            Nenhum contato encontrado.
                                                        </Col>
                                                    )}

                                                    <Col span={24}>
                                                        <Button className="mt-1 mb-2 w-full" type="dashed" onClick={() => setShowNewUser(true)}>
                                                            Novo contato
                                                        </Button>
                                                    </Col>
                                                </>
